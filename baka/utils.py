...
async def log_to_channel(bot: Bot, event_type: str, details: dict):
    ...
    headers = {
        "start": "🎀 <b>ʙσᴛ Ꝛє-ᴅєᴘʟσʏєᴅ!</b>",
        "join": "🎉 <b>ηєᴡ ɢꝛσυᴘ!</b>",
        "leave": "💔 <b>ʟєғᴛ ɢꝛσυᴘ...</b>",
        "command": "👮‍♀️ <b>ᴧᴅϻɪη ᴧʟєꝛᴛ</b>",
        "transfer": "💸 <b>ϻσηєʏ ϻσνєs</b>"
    }
    header = headers.get(event_type, "🔔 <b>ʟσɢ</b>")
    
    text = f"{header}\n\n⌚ <b>ᴛɪϻє:</b> <code>{now}</code>\n"
    if 'user' in details: text += f"👤 <b>υsєꝛ:</b> {details['user']}\n"
    if 'chat' in details: text += f"🏰 <b>ᴄʜᴧᴛ:</b> {html.escape(details['chat'])}\n"
    if 'action' in details: text += f"🎬 <b>ᴧᴄᴛ:</b> {details['action']}\n"
    if 'link' in details and details['link'] != "No Link": text += f"🔗 <b>ʟɪηᴋ:</b> <a href='{details['link']}'>Tap Here</a>\n"
    text += f"\n🍥 <i>{BOT_NAME} sʏsᴛєϻs</i>"
